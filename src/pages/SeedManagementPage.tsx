  ...
  return (
    ...
    <div className="container mx-auto px-4 py-8 max-w-3xl">
      ...
      <CardHeader className="flex flex-row items-center justify-between space-y-0">
          ...
          <Button variant="ghost" onClick={() => setIsDetailsExpanded(!isDetailsExpanded)} className="transition-transform duration-300 ease-in-out">
            {isDetailsExpanded ? <><ChevronUp className="mr-1 h-4 w-4" /> Hide Details</> : <><ChevronDown className="mr-1 h-4 w-4" /> View Details</>}
          </Button>
        </CardHeader>
          {/* Collapsible Content */}
              ...
              <div className="overflow-x-auto">
                <table className="w-full table-auto">
                    ...
                    <tbody className="divide-y divide-gray-200">
                      ...
                      .filter(([key, value]) => value !== undefined && value !== null && value !== '' && value !== 'N/A' && !urlFields.includes(key))
                      .map(([key, value]) => (
                        <tr key={key}>
