...
import { AlertCircle, Timer, Search, Calendar, Filter } from "lucide-react"
...
export default function EnhancedSummaryTable({ data, title = "Enhanced Summary Table" }: SummaryTableProps) {
  ...
  const processSummaryData = React.useCallback(() => {
    return data.map((item) => {
      ...
      const daysSinceLastWithdrawal = Math.floor((CURRENT_DATE.getTime() - lastWithdrawalDate.getTime()) / (1000 * 60 * 60 * 24));
  ...
  const getWithdrawnAmount = React.useCallback((item: any) => {
  ...
  const filteredData = React.useMemo(() => {
    ...
    if (statusFilter !== "all") {
      filtered = filtered.filter(item => {
        const status = getItemStatus(item.remainingVolume, item.daysSinceLastWithdrawal, item.daysUntilExpiry, item.withdrawnYTD)
        switch (statusFilter) {
          case "critical":
  ...
  // Determine item status based on remaining volume threshold
  const getItemStatus = (volume: number, daysSinceWithdrawal: number, daysUntilExpiry: number, withdrawnYTD: number) => {
    const isVeryLowVolume = volume < VERY_LOW_VOLUME_THRESHOLD
    const isLowVolume = volume < LOW_VOLUME_THRESHOLD
        ...
        <div className="flex flex-col space-y-3">
          <div>
            ...
            <CardDescription className="text-sm">
              Monitor crop seed inventory and usage
                    ...
                    return (
                      <TableRow
                        ...
                        className={clsx(
                          ...
                          status.isWarning && !status.isCritical && "bg-yellow-50 hover:bg-yellow-100 border-l-4 border-l-yellow-500",
